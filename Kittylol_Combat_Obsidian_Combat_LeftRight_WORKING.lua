...
--==========================================================================
--  KITTYLOL / OBSIDIAN UI
--  Load the UI before any feature controls are created.
--  No splash/intro screen is used.
--==========================================================================
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/kittylol-hub/Kittylol/refs/heads/main/Library.lua"))()
-- Obsidian runtime bridge: all combat runtime checks use Options.*
...
NotifyUnsupportedFeature = function(featureName)
    Notify({
        Title = featureName,
        Content = "Your executor doesn't support this feature",
        Duration = 3
    })
end
...
-- Combat layout:
-- LEFT  = Aimbot / Checks / Anti-Katana / Status
-- RIGHT = Silent Aim / Triggerbot / Orbit


local AimbotSection = CombatTab:AddLeftGroupbox("Aimbot")
AimbotSection:AddToggle("aimbot_enabled", {
    Text = "Enabled",
    Default = false,
    Callback = function(v)
...
AimbotSection:AddSlider("max_distance", {
    ...
    Rounding = 0,
    Callback = function(v)
        AimbotSettings.MaxDistance = tonumber(v) or 1000
    end
})
AimbotSection:AddSlider("aimbot_smoothing", {
    ...
    Rounding = 1,
    Callback = function(v)
        AimbotSettings.Smoothing = tonumber(v) or 5
    end
})
...
AimbotSection:AddLabel("Aim Key"):AddKeyPicker("aimbot_bind", {
    Default = "MB2",
...
ChecksSection:AddToggle("team_check", {
    Text = "Team Check",
    Default = true,
    Callback = function(v)
...
ChecksSection:AddToggle("show_fov", {
    Text = "Show FOV",
    Default = false,
    Callback = function(v)
        if FOVCircle then
...
ChecksSection:AddSlider("fov_slider", {
    Text = "Field of View",
    Default = 90,
    Min = 0,
    Max = 1000,
    Rounding = 0,
    Callback = function(v)
        State.FOVRadius = tonumber(v) or 90
    end
})
...
TriggerbotSection:AddLabel("Hold Key"):AddKeyPicker("triggerbot_bind", {
    Default = "MB2",
    SyncToggleState = false,
    Mode = "Hold",
    Text = "Hold Key",
    Callback = function(Value)
        State.TriggerbotActive = Value
    end
})
...
local OrbitSection = CombatTab:AddRightGroupbox("Orbit")
...
local AntiKatanaSection = CombatTab:AddLeftGroupbox("Anti-Katana")
...
local StatusSection = CombatTab:AddLeftGroupbox("Status")
